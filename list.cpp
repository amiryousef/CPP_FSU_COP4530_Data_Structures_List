...
// sorts and other macro mutators

template < typename T >
template < class P >
void List<T>::Sort (const P& comp)
...
template < typename T >
template < class P >
void List<T>::Merge (List<T>& y, const P& p)
...
{
  // TBS
  // use only two extra variables - suggest these:
  // Link * link, * temp;
}
...
template < typename T >
List<T>::Link::Link (const T& Tval) : Tval_(Tval), prev_(0), next_(0)
// Link constructor
{}
...
{
  if (location == 0 || newLink == 0) return;
  // TBS
}
...
template < typename T >
List<T>::List (const List<T>& x) : head_(0), tail_(0)
// copy constructor
{
  Init();
  Append(x);
...
// default constructor
template < typename T >
ConstListIterator<T>::ConstListIterator ()  :  curr_(0)
